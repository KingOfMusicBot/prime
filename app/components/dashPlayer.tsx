...
const VideoPlayer: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const shakaInitTimeout = setTimeout(() => {
      if (src.startsWith("blob:") || src.startsWith("data:") || src.startsWith("local-device-")) {
        ...
        if (
          ...
          if (!fallbackApplied && drmConfig?.clearKeys) {
            ...
            try {
              ...
              const savedTime = ContentId ? parseFloat(localStorage.getItem(`lectureTime_${ContentId}`) || "0") : 0;
              await newPlayer.load(src, savedTime, "application/dash+xml");
      ...
      const loadVideo = async () => {
        try {
          if (isOffline) {
            ...
            const item = list.find((x: any) => x.offlineUri === src);
            if (item?.appMetadata?.drmConfig?.clearKeys) {
              player.configure({
                drm: {
                  servers: { "org.w3.clearkey": "" },
                  clearKeys: item.appMetadata.drmConfig.clearKeys
                }
              });
            }
            await storage.destroy();
            const savedTime = ContentId ? parseFloat(localStorage.getItem(`lectureTime_${ContentId}`) || "0") : 0;
            await player.load(src, savedTime);
          } else {
            ...
            const savedTime = ContentId ? parseFloat(localStorage.getItem(`lectureTime_${ContentId}`) || "0") : 0;
            await player.load(src, savedTime, "application/dash+xml");
          }
          ...
          if (isOffline) {
            toast.error("Failed to play downloaded video.");
          }
        }
      };
